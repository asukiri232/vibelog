...
## 6. Медиа (фото и видео)

**Переменная:** `SERVE_MEDIA=true` на web-сервисе (обязательно).

При сборке файлы из `mysite/media/` копируются в `deploy/media_seed`. При старте, если папка media пустая (часто из‑за Volume), они восстанавливаются автоматически.

### Volume (опционально)

Нужен только для **новых** загрузок после деплоя. Если подключили Volume и картинки пропали — сделайте **Redeploy** (скрипт `ensure_media.sh` зальёт файлы из seed).

1. Web-сервис → **Volumes** → **Add Volume**  
2. Mount Path: `/app/mysite/media`  
3. Redeploy

Проверка: `/ready/` → должно быть `media_files=150` (примерно), не `media_files=0`.
